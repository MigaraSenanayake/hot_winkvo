...
if (isset($_POST['submitOrder'])) {
     if (isset($_POST['cusId']) && !empty($_POST['cusId']) && isset($_SESSION['order_items']) && !empty($_SESSION['order_items'])) {
        ...
        if ($stmt->execute()) {
            ...
            try {
                $connector = new WindowsPrintConnector($selectedPrinter); // Use the selected printer name
                ...
                redirect("create_order2.php", "Error printing KOT: " . $e->getMessage());
        ...
        if (!isset($_POST['cusId']) || empty($_POST['cusId'])) {
            ...
            redirect("create_order2.php", "Customer not selected. Please select a customer.");
            
        } elseif (empty($_SESSION['order_items'])) {
            
            redirect("create_order2.php", "No items in the cart. Please add items to the cart.");
