...
if (isset($_POST['addC_Item'])) {
    ...
    if (!empty($citemName) && is_numeric($cQty) && $cQty > 0) {
        ...
        redirect("create_order.php", $citemName . ' added to Cart');
    } else {
        redirect("create_order.php", "Invalid custom item details. Please check your input.");
    }
}
...
if (isset($_POST['submitOrder'])) {
    if (
        ...
        $currentTime24 = date('Y-m-d H:i:s'); // Order creation 
        ...
        $remarks = validate($_POST['remarks']); // Special 
        ...
        $stmt = $conn->prepare(
            "INSERT INTO w_orders (customer_id, total_price, created_by, created_at, serve_location, table_or_room, ready_time, remarks) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
        );
            ...
            foreach ($_SESSION['order_items'] as $item) {
                ...
                if (isset($item['item_id']) && !empty($item['item_id'])) {
                    $stmtItem = $conn->prepare(
                        "INSERT INTO w_order_items (order_id, item_id, quantity, unit_price, total_price) 
                        VALUES (?, ?, ?, ?, ?)"
                    );
                    $stmtItem->bind_param("iiidd", $orderId, $item['item_id'], $item['qty'], $item['price'], $totalItemPrice);
                } else {
                    $stmtItem = $conn->prepare(
                        "INSERT INTO w_order_items (order_id, item_id, name, quantity, unit_price, total_price) 
                        VALUES (?, NULL, ?, ?, ?, ?)"
                    );
                ...
                if (!$stmtItem->execute()) {
                    redirect("create_order.php", "Error adding item: " . $stmtItem->error);
                    exit();
                }
            }
        ...
        // Your existing code to print the KOT content
            ...
            redirect("create_order.php", "Error while saving the order: " . $stmt->error);
        }
    } else {
        ...
        redirect("create_order.php", trim($errorMessage));
    }
}
?>
