...
if ($paramResult === null) {
    redirect('create_order2.php', 'Parameter is missing');
    exit();
} elseif (!is_numeric($paramResult)) {
    redirect('create_order2.php', 'Parameter is not numeric');
    exit();
}
...
if (isset($_SESSION['order_items'][$indexValue])) {
    ...
    if ($isCustomItem) {
        ...
        redirect('create_order2.php', 'Custom item removed successfully');
    } else {
        ...
        // Redirect with success message
        redirect('create_order2.php', 'Standard item removed successfully');
    }
} else {
    // If the item at the index does not exist
    redirect('create_order2.php', 'Item not found or already removed');
}
