...
<div class="container-fluid px-3">
    <div class="card mt-2 shadow-sm">
        <div class="card-header">
            <h4 class="mb-0">Edit In-House Guest
            ...
            <form action="../code.php" method="POST" enctype="multipart/form-data">
                ...
                if (isset($_GET['id'])) {
                    ...
                    if (!empty($b_cus_Id)) {
                        ...
                        if ($b_cus_Data && $b_cus_Data['status'] == 200) {
                            ...
                            // Convert room_category and room_type strings into arrays
                            $b_customer['room_category'] = isset($b_customer['room_category']) ? explode(',', $b_customer['room_category']) : [];
                            $b_customer['room_type'] = isset($b_customer['room_type']) ? explode(',', $b_customer['room_type']) : [];
                        } else {
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-4">
                        ...
                        <input type="date" name="arrival" value="<?= htmlspecialchars($b_customer['arrival'] ?? ''); ?>" required class="form-control" />
                    </div>
                    <div class="col-md-4 mb-4">
                        <label for="departure-date">Departure Date *</label>
                        <input type="date" id="departure-date" name="departure" value="<?= htmlspecialchars($b_customer['departure'] ?? ''); ?>" required class="form-control" />
                    </div>
                    <div class="col-md-4 mb-4">
                        <label for="r_num">Number of Rooms *</label>
                        <input type="number" name="r_num" value="<?= htmlspecialchars($b_customer['r_num'] ?? ''); ?>" required class="form-control" min="1" />
                    </div>
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <select name="room_pack[]" class="form-select js-mySelect2" multiple required>
                            <?php
                                ...
                                // Assume $selectedRoomPacks is an array of already selected room package IDs
                                $selectedRoomPacks = isset($b_customer['room_pack']) ? explode(',', $b_customer['room_pack']) : [];
                               
                                if ($name && mysqli_num_rows($name) > 0) {
                                    foreach ($name as $nameItems) {
                                        // Check if this room package ID is in the selected array
                                        $selected = in_array($nameItems['id'], $selectedRoomPacks) ? 'selected' : '';
                                        echo '<option value="'.$nameItems['id'].'" '.$selected.'>'.$nameItems['id'].' - '.$nameItems['cat_code'].'</option>';
                                    }
                                } else {
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <select name="room[]" class="form-select js-mySelect2" multiple required>
                            <?php
                            $rooms = getAll('room');
                            $selectedRooms = isset($b_customer['room']) ? explode(',', $b_customer['room']) : [];
                            $selectedRooms = array_map('trim', $selectedRooms); // Ensure no whitespace issues

                            if ($rooms && mysqli_num_rows($rooms) > 0) {
                                foreach ($rooms as $room) {
                                    $roomNo = htmlspecialchars($room['room_no'], ENT_QUOTES, 'UTF-8');
                                    $selected = in_array($roomNo, $selectedRooms) ? 'selected' : '';
                                    echo "<option value=\"{$roomNo}\" {$selected}>{$roomNo}</option>";
                                }
                            } else {
                    ...
                    <div class="col-md-4 mb-4">
                        ...
                        <input type="number" name="a_count" value="<?= htmlspecialchars($b_customer['a_count'] ?? ''); ?>" required class="form-control" min="1"/>
                    </div>

                    <div class="col-md-4 mb-4">
                        <label for="c_count_s">Children (Below 6 years) *</label>
                        <input type="number" name="c_count_s" value="<?= htmlspecialchars($b_customer['c_count_s'] ?? ''); ?>" class="form-control" min="0"/>
                    </div>
                    <div class="col-md-4 mb-4">
                        <label for="c_count_b">Children (Between 6 - 12) *</label>
                        <input type="number" name="c_count_b" value="<?= htmlspecialchars($b_customer['c_count_b'] ?? ''); ?>" class="form-control" min="0"/>
                    </div>
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="text" name="advance" value="<?= htmlspecialchars($b_customer['advance'] ?? ''); ?>"  class="form-control" />
                    </div>
                    <div class="col-md-12 mb-4">
                ...
                <div class="row mt-3 mb-3">
                    <div class="col-md-12">
                        <button type="submit" name="edit_b_cus" class="btn btn-outline-success float-end">Update</button>
                    </div>
                </div>
            </form>
        </div>
