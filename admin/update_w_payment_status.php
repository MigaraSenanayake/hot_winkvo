...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['payment_ids']) && is_array($_POST['payment_ids'])) {
        ...
        if (!empty($payment_ids)) {
            ...
            $query = "UPDATE `w_payments` SET `status` = 'Success' WHERE `id` IN ($payment_ids_str)";
            $result = mysqli_query($conn, $query);

            if ($result) {
                // Success: redirect with a success message
                redirect('verify_w_payments.php', 'Payment statuses updated successfully.');
            } else {
                // Log error and redirect with failure message
                error_log(mysqli_error($conn));
                redirect('verify_w_payments.php', 'Failed to update payment statuses.');
            }
        } else {
            // Redirect with no payments selected message
            redirect('verify_w_payments.php', 'Payments not selected.');
        }
    } else {
        // Redirect with no payments selected message
        redirect('verify_w_payments.php', 'Payments not selected.');
