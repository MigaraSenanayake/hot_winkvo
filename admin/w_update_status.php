...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['order_ids']) && is_array($_POST['order_ids'])) {
        ...
        if (!empty($order_ids)) {
            ...
            $query = "UPDATE `w_orders` SET `status` = 'paid' WHERE `id` IN ($order_ids_str)";
            $result = mysqli_query($conn, $query);

            if ($result) {
                // Success: redirect with a success message
                redirect('verify_w_order_payments.php', 'Order statuses updated successfully.');
            } else {
                // Log error and redirect with failure message
                error_log(mysqli_error($conn));
                redirect('verify_w_order_payments.php', 'Failed to update order statuses.');
            }
        } else {
            // Redirect with no orders selected message
            redirect('verify_w_order_payments.php', 'No orders selected.');
        }
    } else {
        // Redirect with no orders selected message
        redirect('verify_w_order_payments.php', 'No orders selected.');
